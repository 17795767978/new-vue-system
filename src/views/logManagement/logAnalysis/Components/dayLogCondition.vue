...
export default {
  ...
  mounted () {
    ...
    this._getLoginLogStatisticAnalysis({
  ...
  methods: {
    _getLoginLogStatisticAnalysis (params) {
      this.loading = true
      this.$api['wholeInformation.loginLogStatisticAnalysis'](params).then(res => {
        this.loading = false
        this.title = {}
        this.lineData = [{
          name: '登录用户数',
        ...
        this.yData = [
          {
            ...
            splitNumber: (() => {
              ...
              if (res.datas[0].length > 0) {
                let maxData = Math.max(Math.max(res.datas[0]), Math.max(res.datas[1]))
                if (maxData >= 10) {
                  return ''
                } else if (maxData % 2 === 0) {
                  return maxData / 2
