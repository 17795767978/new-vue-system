<template>
  <div class="indi-light">
    <div>
      <span v-for="(item, index) in getDataTop" :key="index">
        <img :title="item.name" :src="setIcon(item, item.status, index)"/>
      </span>
    </div>
    <div>
      <span v-for="(item, index) in getDataBot" :key="index">
...
export default {
  ...
  methods: {
    ...
    setData (id, status, typeName) {
      let data = this.indiData[id]
      data['status'] = status
      data['name'] = typeName
      this.$set(this.indiData, id, data)
    }
  },
  data () {
    return {
