...
export function axiosRequestSucessFunc (config) {
  ...
  if (config.data && config.data.orgId) {
    config.data.orgId = config.data.orgId === '1' ? '' : config.data.orgId
  }
  if (config.data && config.data.orgUuid) {
    config.data.orgUuid = config.data.orgUuid === '1' ? '' : config.data.orgUuid
  }
  return config
}
